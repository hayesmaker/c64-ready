...
export function domKeyToC64Actions(
  ...
  switch (k) {
    ...
    case 'tab':      main(C64_KEY.CTRL);        break;
    case 'control':  main(C64_KEY.COMMODORE);   break;
    case 'capslock': main(C64_KEY.COMMODORE);   break;
...
export class EmulatorInput {
  ...
  private handleKeyDown(event: KeyboardEvent): void {
    ...
    if (this.inputMode === 'mixed') {
      ...
      if (event.code in MIXED_JOYSTICK_KEYS) {
        ...
        this.emulator.joystickPush(this.keyboardJoystickPort, dir as import('./constants').JoystickInput);
        event.preventDefault();
      } else {
  ...
  private handleKeyUp(event: KeyboardEvent): void {
    ...
    if (this.inputMode === 'mixed') {
      if (event.code in MIXED_JOYSTICK_KEYS) {
        const dir = MIXED_JOYSTICK_KEYS[event.code];
        this.emulator.joystickRelease(this.keyboardJoystickPort, dir as import('./constants').JoystickInput);
        event.preventDefault();
      } else {
        const acts = domKeyToC64Actions(event.key, event.shiftKey, 'keyup');
        if (acts.length > 0) {
