...
var C64Joystick = function() {
  ...
  this.portEnabled = [false, false]
  this.mousePortEnabled = [false, false]

  const storageKeyUp = window.localStorage.getItem('joyUp')
  const storageKeyDown = window.localStorage.getItem('joyDown')
  const storageKeyLeft = window.localStorage.getItem('joyLeft')
  const storageKeyRight = window.localStorage.getItem('joyRight')
  const storageKeyFire = window.localStorage.getItem('joyFire')

  const keyUp = storageKeyUp ? storageKeyUp : 'arrowup'
  const keyDown = storageKeyDown ? storageKeyDown : 'arrowdown'
  const keyLeft = storageKeyLeft ? storageKeyLeft : 'arrowleft'
  const keyRight = storageKeyRight ? storageKeyRight : 'arrowright'
  const keyFire = storageKeyFire ? storageKeyFire : 'z'

  this.joystickKeys = [
    [false, keyUp, false, keyLeft, keyRight, false, keyDown, false, keyFire, 'x'],
    [false, keyUp, false, keyLeft, keyRight, false, keyDown, false, keyFire, 'x']
  ]
  this.joystickButtons = [1, 1]
  this.joystickButtonActions = [['fire', 'up'], ['fire', 'up']]
  ...
  this.dialogJoystickButtonActions = [['fire', 'up'], ['fire', 'up']]
...
C64Joystick.prototype = {
  ...
  init: function() {
    ...
    console.log('C64Joystick::init defaultGamepadPort:', _this.defaultGamepadPort);
    window.addEventListener('gamepadconnected', function(event) {
      var buttons = []
      for (var i = 0; i < 16; i++) {
        buttons[i] = false
      }
      console.log('c64js::gamepadconnected', event.gamepad, 'Port: ', _this.defaultGamepadPort);
      _this.gamepads[event.gamepad.id] = {
        port: _this.defaultGamepadPort,
        index: event.gamepad.index, up: false, down: false, left: false, right: false, buttons: buttons
      }
    })
    window.addEventListener('gamepaddisconnected', function(event) {
      console.log('c64js::gamepaddisconnected', event.gamepad);
      delete _this.gamepads[event.gamepad.id]
    })
  },

  redefineKey: function(joyAction, keyValue) {
    let index = -1
    switch (joyAction) {
      case 'joyUp':
        index = 1
    ...
    const joyIndex = this.portEnabled.indexOf(true);
    this.joystickKeys[joyIndex].splice(index, 1, keyValue);
  },

  redefineKeys: function(keyUp, keyDown, keyLeft, keyRight, keyFire) {
    this.joystickKeys = [
      [false, keyUp, false, keyLeft, keyRight, false, keyDown, false, keyFire, 'x'],
      [false, keyUp, false, keyLeft, keyRight, false, keyDown, false, keyFire, 'x']
    ]
  ...
  setPortEnabled: function(port, enabled) {
    this.defaultGamepadPort = port;
    if (port === 1) {
      this.portEnabled[0] = enabled
      this.portEnabled[1] = false
    }
    if (port === 2) {
      this.portEnabled[1] = enabled
      this.portEnabled[0] = false
    }
  },
  ...
  buildSettingsDialog: function() {
    ...
    this.settingsDialog = UI.create('UI.Dialog', {
      'id': 'c64JoystickSettings', 'title': 'Joystick Settings', 'width': width, 'height': height
    })
    var html = ''
    html = '<div>'
...
C64OnscreenJoystick.prototype = {
  ...
  processButtonTouches: function(touches) {
    ...
    for (var i = 0; i < touches.length; i++) {
      ...
      var distance1 = (this.button1X - x) * (this.button1X - x) + (this.button1Y - y) * (this.button1Y - y)
      if (distance1 < this.button1Radius * this.button1Radius) {
        button1Down = true
      }
      var distance2 = (this.button2X - x) * (this.button2X - x) + (this.button2Y - y) * (this.button2Y - y)
      if (distance2 < this.button2Radius * this.button2Radius) {
        button2Down = true
      }
    }
  ...
  processJoystickTouches: function(touches) {
    ...
    for (var i = 0; i < touches.length; i++) {
      ...
      var x = touch.pageX - elementOffset(this.canvas).left
      var y = touch.pageY - elementOffset(this.canvas).top
      var distance = (this.joystickX - x) * (this.joystickX - x) + (this.joystickY - y) * (this.joystickY - y)
  ...
  draw: function() {
    ...
    this.canvas.width = holder.clientWidth - 4 //UI.getScreenWidth() - 4;
    this.context = this.canvas.getContext('2d')
...
C64OnscreenKeyboard.prototype = {
  ...
  processStartTouches: function(touches) {
    ...
    for (var i = 0; i < touches.length; i++) {
      ...
      var x = touch.pageX - elementOffset(this.canvas).left
      var y = touch.pageY - elementOffset(this.canvas).top
      for (var j = 0; j < this.keyPositions.length; j++) {
        var keyX = this.keyPositions[j].x
        var keyY = this.keyPositions[j].y
        var keyWidth = this.keyPositions[j].width
        var keyHeight = this.keyPositions[j].height
        if (x > keyX && x < keyX + keyWidth && y > keyY && y < keyY + keyHeight) {
          var keyIndex = this.keyPositions[j].keyIndex
          if (keyIndex === C64_KEY_SHIFT_LEFT || keyIndex === C64_KEY_SHIFT_RIGHT) {
            keyWasShift = false
          }
    ...
    for (j = 0; j < this.keysDown.length; j++) {
      if (this.keysDown[j] === C64_KEY_SHIFT_LEFT || this.keysDown[j] === C64_KEY_SHIFT_RIGHT) {
        shiftIsDown = true
        break
      }
    }
  ...
  processTouches: function(touches) {
    ...
    for (var i = 0; i < touches.length; i++) {
      ...
      for (var j = 0; j < this.keyPositions.length; j++) {
        ...
        if (x > keyX && x < keyX + keyWidth && y > keyY && y < keyY + keyHeight) {
          var keyIndex = this.keyPositions[j].keyIndex
          if (keyIndex !== C64_KEY_SHIFT_LEFT && keyIndex !== C64_KEY_SHIFT_RIGHT && keyIndex !== C64_KEY_COMMODORE && keyIndex !== C64_KEY_CTRL) {
            keysDown.push({ keyIndex: keyIndex, identifier: touch.identifier })
          }
          break
        }
    ...
    for (i = 0; i < this.keysDown.length; i++) {
      if (this.keysDown[i] !== C64_KEY_SHIFT_LEFT && this.keysDown[i] !== C64_KEY_SHIFT_RIGHT && this.keysDown[i] !== C64_KEY_COMMODORE && this.keysDown[i] !== C64_KEY_CTRL) {
        var found = false
        for (j = 0; j < keysDown.length; j++) {
...
*/
    ...
    for (var i = 0; i < this.keysDown.length; i++) {
      if (this.keysDown[i] == C64_KEY_SHIFT_LEFT || this.keysDown[i] == C64_KEY_SHIFT_RIGHT) {
        this.shiftDown = true
      }
    ...
    */
    ...
    for (var y = 0; y < 6; y++) {
      ...
      for (i = 0; i < keys[y].length; i++) {
        ...
        if (keyDown) {
          this.context.fillRect(keys[y][i].left, keyboardPositionY + keys[y][i].top, keyWidth * keys[y][i].width * scale, keyHeight * scale)
        }
        ...
        this.context.moveTo(Math.floor(x * keyWidth * scale) + 0.5, keyboardPositionY + Math.floor(y * keyHeight * scale) + 0.5)
        this.context.lineTo(Math.floor(x * keyWidth * scale) + 0.5, keyboardPositionY + Math.floor((y + 1) * keyHeight * scale) + 0.5)
        x += keys[y][i].width
      }
      this.context.moveTo(Math.floor(x * keyWidth * scale) + 0.5, keyboardPositionY + Math.floor(y * keyHeight * scale) + 0.5)
      this.context.lineTo(Math.floor(x * keyWidth * scale) + 0.5, keyboardPositionY + Math.floor((y + 1) * keyHeight * scale) + 0.5)
    ...
    for (var row = 0; row < keys.length; row++) {
      ...
      for (i = 0; i < keys[row].length; i++) {
        ...
        character = this.keyboard[row][i].charCode
        if (this.shiftDown) {
...
C64Sound.prototype = {
  init: function() {
    console.log('c64.js::C64Sound.init')
    this.loadPrefs();

    if (ENVIRONMENT_HAS_NODE) {
      console.log('c64.js::Running in Node.js environment. Attempting Audio init');
      this.setModel('6581');
    }
  },
...
//      console.log('start audio!');
  ...
  volumeChange: function(newVol) {
    if (this.audioCtx) {
      this.gainNode.gain.setValueAtTime(newVol / 100, this.audioCtx.currentTime)
    }
  },

  startAudio2: async function() {
    // this.audioCtx = new AudioContext();
    await audioContext.audioWorklet.addModule('/c64page/js/wasm-worklet-processor.js')
    this.audioNode = new AudioWorkletNode(audioContext, 'wasm-worklet-processor');
  ...
  startAudio: function() {
    // this.startAudio2();
    console.log('c64.js::C64Sound.startAudio')
    this.legacyStartAudio();
  },

  stopAudio: function() {
    this.legacyStopAudio();
  },

  audioProcess: function(e) {
    // console.log('c64.js::C64Sound.audioProcess', ENVIRONMENT_HAS_NODE);
    var outBuffer = e.outputBuffer
    var channelData = outBuffer.getChannelData(0)

    if (ENVIRONMENT_HAS_NODE ||
      (debugger_isRunning() && !document.hidden) &&
      ...
      // @c64live Headless mode audio hook:
      try {
        if (typeof globalThis !== 'undefined' && typeof globalThis.__onC64Audio === 'function') {
          globalThis.__onC64Audio(ptr, this.audioBufferLength, this.sampleRate || 44100)
        } else if (typeof Module !== 'undefined' && typeof Module.onC64Audio === 'function') {
          Module.onC64Audio(ptr, this.audioBufferLength, this.sampleRate || 44100)
        }
      } catch (e) { }
    } else {
  ...
  legacyStartAudio: function() {
    console.log('c64.js::C64Sound.legacyStartAudio')
    var AudioContext = window.AudioContext || webkitAudioContext || false
    if (!AudioContext) {
    ...
    var _this = this
    // need to stop audio if it was already started,
    // otherwise we might end up with multiple audio contexts running
    this.stopAudio();
    // try {
    ...
    var defaultVol = 75;
    var newVol = parseInt(localStorage.getItem('volume')? localStorage.getItem('volume') : defaultVol)
    this.gainNode.gain.setValueAtTime(newVol / 100, this.audioCtx.currentTime)
    ...
    // Headless hook: expose scriptNode so runner can trigger audio manually
    if (typeof globalThis !== 'undefined') {
      globalThis.__c64ScriptNode = this.scriptNode;
    }
    this.scriptNode.onaudioprocess = function(e) {
      // console.log('c64.js::C64Sound.legacyStartAudio::onaudioprocess')
  ...
  legacyStopAudio: function() {
    if (this.gainNode) {
      this.gainNode.disconnect(this.audioCtx.destination)
      this.gainNode.onaudioprocess = null
      this.gainNode = null
    }
    if (this.audioCtx) {
...
if (ENVIRONMENT_HAS_NODE) {
  // @c64Live Expose startAudio to global scope for headless mode
  global.startAudio = C64Sound.prototype.startAudio.bind(C64Sound.prototype);
}


var C64Colors = function() {
  this.colors = [0xFF000000, 0xFFffffff, 0xFF383381, 0xFFc8ce75, 0xFF973c8e, 0xFF4dac56, 0xFF9b2c2e, 0xFF71f1ed, 0xFF29508e, 0xFF003855, 0xFF716cc4, 0xFF4a4a4a, 0xFF7b7b7b, 0xFF9fffa9, 0xFFeb6d70, 0xFFb2b2b2]
}
...
C64Colors.prototype = {
  ...
  resetColor: function(colorIndex) {
    var colors = [0xFF000000, 0xFFffffff, 0xFF383381, 0xFFc8ce75, 0xFF973c8e, 0xFF4dac56, 0xFF9b2c2e, 0xFF71f1ed, 0xFF29508e, 0xFF003855, 0xFF716cc4, 0xFF4a4a4a, 0xFF7b7b7b, 0xFF9fffa9, 0xFFeb6d70, 0xFFb2b2b2]
...
var C64 = (function() {
  ...
  return (function(C64) {
    ...
    // Headless runner hook: expose heap
    if (typeof globalThis !== 'undefined') {
      globalThis.__c64_HEAPU8 = Module['HEAPU8'];
      globalThis.__c64_HEAPF32 = Module['HEAPF32'];
    }
    ...
    function createWasm() {
      wasmBinary = wasmBinary || (typeof global !== 'undefined' && global.wasmBinary) || (typeof Module !== 'undefined' && Module.wasmBinary);
    ...
    function emscripten_realloc_buffer(size) {
      try {
        ...
        console.log('emscripten_realloc_buffer error', e)
      }
    }
...
c64['onRuntimeInitialized'] = function() {
  ...
  console.log('c64::onRuntimeInitialized')
  ...
  if (model == 'pal') {
    console.log('Using PAL model')
    c64_setModel(1)
  } else if (model == 'ntsc') {
    console.log('Using NTSC model')
    c64_setModel(2)
  }
...
function pauseOn() {
  cancelAnimationFrame(c64animationInterval)
  isPaused = true
}
...
var D64Util = {}
D64Util.trackInfo = [{ track: 0, numSectors: 0, sectorsIn: 0, d64Offset: 0x00000 }, // for 1-based track indexing
...
//          throw new Error('Unknown directory entry file type');
      ...
      var name = ''
      for (i = 0; i < petsciiName.length; i++) {
        name += String.fromCharCode(petsciiName[i])
      }


      dirEntries.push({
        type: fileType, //        petsciiName,
//        screencodeName: petsciiToScreenArray(petsciiName),
        name: name, d64FileOffset: offset + 0x05, track: track, sector: sector, sectors: sectors
      })
    }
...
// @c64live headless mode ignore this
console.info('C64 Loading in Headless runner', ENVIRONMENT_HAS_NODE);
// cross-env high-resolution timestamp
let getTimestamp;

if (typeof globalThis !== 'undefined' && globalThis.performance && typeof globalThis.performance.now === 'function') {
  // Browser or environment with performance API
  getTimestamp = () => globalThis.performance.now();
} else if (typeof process !== 'undefined' && typeof process.hrtime === 'function') {
  // Node: use high-resolution monotonic clock
  // process.hrtime.bigint gives nanoseconds as BigInt (Node >=10.7). Convert to ms.
  if (typeof process.hrtime.bigint === 'function') {
    getTimestamp = () => Number(process.hrtime.bigint() / 1000000n);
  } else {
    // Older Node: process.hrtime() returns [sec, nanosec]
    getTimestamp = () => {
...
globalThis.__resetC64Clock = function () {
  g_lastUpdate = getTimestamp();
  globalThis.__frameOutCount = 0;
...
function snapshotBase64ToBuffer(str) {
  const base64 = str.replace('data:application/octet-stream;base64,', '');
  var a = atob(base64);
  var buf = new Uint8Array(a.length)

  for (var i = 0, l = buf.length; i < l; i++) {
    buf[i] = a.charCodeAt(i)
  }
  return buf
}

function base64ToBuffer(str, doAtob = true) {
  var b = doAtob? atob(str) : str;
...
document.addEventListener('audiotoggle', function() {
  console.log('audio toggle', this.audioEnabled)
  c64.sound.toggleAudio()
})

document.addEventListener('audiostart', function() {
  console.log('audio start::', this.audioEnabled)
  c64.sound.startAudio()
})

document.addEventListener('audiostop', function() {
  console.log('audio stop::', this.audioEnabled)
  c64.sound.stopAudio()
})


document.addEventListener('contentstart', function(e) {
  doSomething()
  addKeyEvents()
  if (!g_started) {
    console.log('contentstart::debug', e.detail.gameId)
    g_gameId = e.detail.gameId
    startContent()
  }
})

document.addEventListener('contentleave', function() {
  if (c64animationInterval) {
    cancelAnimationFrame(c64animationInterval)
    c64animationInterval = null
  }
  removeKeyEvents()
  c64.sound.stopAudio()
})

document.addEventListener('emulatorpause', function() {
  pauseOn()
})

document.addEventListener('emulatorresume', function() {
  pauseOff()
})
...
document.getElementById('c64-holder').addEventListener('fullscreenchange', function() {
...
if (ENVIRONMENT_HAS_NODE) {
  global.startContentHeadless = function() {
    console.log('c64_getModel:', c64_getModel());
    doSomething();
    startContent();
    console.log('c64_getModel:', c64_getModel());
...
function startContent() {
  console.log('c64.js::startContent called');
  c64.sound.checkAudio()
  // c64_setModel(0);

  // @todo remove cursor in game not when content started
  // maybe add a timer
  ...
  if (typeof g_snapshot !== 'undefined' && g_snapshot) {
    const snapshotData = base64ToBuffer(g_snapshot)
    c64_loadSnapshot(snapshotData, snapshotData.length)
  }

  if (typeof g_snapshotData !== 'undefined' && g_snapshotData) {
    console.log('c64::snapshotData', g_snapshotData);
    const snapshotData = snapshotBase64ToBuffer(g_snapshotData)
    c64_loadSnapshot(snapshotData, snapshotData.length)
  }
...
function redraw() {
  frames++;
  // if (frames % 50 === 0) {
  //   // console.log('c64js:: Frame redraw', frames, '[c64_getModel]', c64_getModel());
  // }
  ...
  // DEBUG: log what pixel buffer pointer we're getting
  // if (frames % 50 === 0) {
  //   // console.log('c64js:: redraw ptr=' + ptr + ' len=' + len);
  // }
  ...
  context.drawImage(offscreenCanvas, 0, 0, offscreenCanvas.width, offscreenCanvas.height, 0, 0, canvas.width, canvas.height)

  // @c64live headless mode hook
 try {
   if (typeof globalThis !== 'undefined' && typeof globalThis.__onC64Frame === 'function') {
      globalThis.__onC64Frame(ptr, len, offscreenCanvas.width, offscreenCanvas.height)
   }
 } catch (e) {}
  ...
  if (!g_started) {
    ...
    if (g_c64Settings.mobileJoystick && !isMobile && typeof arrowKeysImage.naturalWidth != 'undefined') {
      ...
      context.drawImage(controlsImage, controlsImageLeft, controlsImageTop, controlsImage.naturalWidth * scale, controlsImage.naturalHeight * scale)
      ...
      if (c64.joystick.hasGamepad()) {
        ...
        context.drawImage(orGamepadImage, orGamepadImageLeft, orGamepadImageTop, orGamepadImage.naturalWidth * scale, orGamepadImage.naturalHeight * scale)
      }
      ...
      context.drawImage(arrowKeysImage, arrowKeysImageLeft, arrowKeysImageTop, arrowKeysImage.naturalWidth * scale, arrowKeysImage.naturalHeight * scale)
      ...
      context.drawImage(zKeyImage, zKeyImageLeft, zKeyImageTop, zKeyImage.naturalWidth * scale, zKeyImage.naturalHeight * scale)
      ...
      if (c64.joystick.getJoystickButtons(0) > 1 || c64.joystick.getJoystickButtons(1) > 1) {
        ...
        context.drawImage(xKeyImage, xKeyImageLeft, xKeyImageTop, xKeyImage.naturalWidth * scale, xKeyImage.naturalHeight * scale)
...
function onC64Ready() {
  console.log('onC64Ready')
  window.dispatchEvent(new Event('c64ready'))
  resize()
  globalThis.__startC64Ticker();
}
...
//  canvas.height = Math.round(c64Height * scale);
  ...
  if (c64_ready) {
    // load / start content?
    console.log('c64.js::resize call redraw')
    redraw()
  }
}
let throttler = 0;
globalThis.__renderCount = 0;
var g_lastHeadlessRender = 0; // @c64live

function render(timestamp) {
  // if (!globalThis.__renderCount) globalThis.__renderCount = 0;
  globalThis.__renderCount++;

  var dTime = timestamp - g_lastUpdate;
  console.log("render dTime (var dTime = timestamp - g_lastUpdate)=", dTime, 'timeStamp', timestamp, 'g_lastUpdate', g_lastUpdate);
  if (!dTime || dTime > 100) {
    // console.log("clamping runaway delta to dTime=", 20);
    dTime = 1000/60; // clamp runaway delta
  }
  ...
  if (c64_ready) {
    ...
    const update = debugger_update(dTime);
    const postUpdateTime = performance.now();
    if (update && !!debugger_isRunning()) {
      globalThis.__frameOutCount++;
      console.log('[FRAME_OUT]', globalThis.__frameOutCount, 'wall_time_ms=', Date.now() - globalThis.__startTime, 'video_fps=', (globalThis.__frameOutCount / ((Date.now() - globalThis.__startTime) / 1000)).toFixed(2));
      redraw();
    }
  }

  if (!ENVIRONMENT_HAS_NODE) {
    c64animationInterval = requestAnimationFrame(render);
  }
}

globalThis.__c64RenderTick = render;
